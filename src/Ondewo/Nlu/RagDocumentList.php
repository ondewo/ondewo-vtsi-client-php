<?php
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# NO CHECKED-IN PROTOBUF GENCODE
# source: ondewo/nlu/rag.proto

namespace Ondewo\Nlu;

use Google\Protobuf\Internal\GPBType;
use Google\Protobuf\Internal\GPBUtil;
use Google\Protobuf\RepeatedField;

/**
 * Response message for listing documents.
 *
 * Generated from protobuf message <code>ondewo.nlu.RagDocumentList</code>
 */
class RagDocumentList extends \Google\Protobuf\Internal\Message
{
    /**
     * List of documents for the current page.
     *
     * Generated from protobuf field <code>repeated .ondewo.nlu.RagDocument documents = 1;</code>
     */
    private $documents;
    /**
     * Total number of documents matching the filter criteria (not limited by pagination).
     *
     * Generated from protobuf field <code>int32 total = 2;</code>
     */
    protected $total = 0;
    /**
     * Page token string (following the format described in <code>RagListDocumentsRequest</code>) for retrieving the next results page
     *
     * Generated from protobuf field <code>string next_page_token = 3;</code>
     */
    protected $next_page_token = '';

    /**
     * Constructor.
     *
     * @param array $data {
     *     Optional. Data for populating the Message object.
     *
     *     @type \Ondewo\Nlu\RagDocument[] $documents
     *           List of documents for the current page.
     *     @type int $total
     *           Total number of documents matching the filter criteria (not limited by pagination).
     *     @type string $next_page_token
     *           Page token string (following the format described in <code>RagListDocumentsRequest</code>) for retrieving the next results page
     * }
     */
    public function __construct($data = NULL) {
        \GPBMetadata\Ondewo\Nlu\Rag::initOnce();
        parent::__construct($data);
    }

    /**
     * List of documents for the current page.
     *
     * Generated from protobuf field <code>repeated .ondewo.nlu.RagDocument documents = 1;</code>
     * @return RepeatedField<\Ondewo\Nlu\RagDocument>
     */
    public function getDocuments()
    {
        return $this->documents;
    }

    /**
     * List of documents for the current page.
     *
     * Generated from protobuf field <code>repeated .ondewo.nlu.RagDocument documents = 1;</code>
     * @param \Ondewo\Nlu\RagDocument[] $var
     * @return $this
     */
    public function setDocuments($var)
    {
        $arr = GPBUtil::checkRepeatedField($var, \Google\Protobuf\Internal\GPBType::MESSAGE, \Ondewo\Nlu\RagDocument::class);
        $this->documents = $arr;

        return $this;
    }

    /**
     * Total number of documents matching the filter criteria (not limited by pagination).
     *
     * Generated from protobuf field <code>int32 total = 2;</code>
     * @return int
     */
    public function getTotal()
    {
        return $this->total;
    }

    /**
     * Total number of documents matching the filter criteria (not limited by pagination).
     *
     * Generated from protobuf field <code>int32 total = 2;</code>
     * @param int $var
     * @return $this
     */
    public function setTotal($var)
    {
        GPBUtil::checkInt32($var);
        $this->total = $var;

        return $this;
    }

    /**
     * Page token string (following the format described in <code>RagListDocumentsRequest</code>) for retrieving the next results page
     *
     * Generated from protobuf field <code>string next_page_token = 3;</code>
     * @return string
     */
    public function getNextPageToken()
    {
        return $this->next_page_token;
    }

    /**
     * Page token string (following the format described in <code>RagListDocumentsRequest</code>) for retrieving the next results page
     *
     * Generated from protobuf field <code>string next_page_token = 3;</code>
     * @param string $var
     * @return $this
     */
    public function setNextPageToken($var)
    {
        GPBUtil::checkString($var, True);
        $this->next_page_token = $var;

        return $this;
    }

}

